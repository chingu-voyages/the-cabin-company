...
const CabinsPage = () => {
    ...
    const searchHandler = () => {
        if (searchCategory === 'pricePerNight') {
            setFilteredCabins(cabins.filter(cabin => +cabin.pricePerNight <= +searchKeyword));
        } else if (searchCategory === 'location') {
            setFilteredCabins(cabins.filter(cabin => {
                return cabin.address.city.toLowerCase().includes(searchKeyword)
    ...
    return (
        <div>
            ...
            <Typography align="center" component="h2">Our Cabins</Typography>
            <Typography align="center" component="h5">Showing {filteredCabins.length} of {cabins.length} cabins for rent</Typography>

            {loading && <LoadingSpinner />}
            {filteredCabins.length === 0 && !loading && <p style={{ color: 'red', textAlign: 'center' }}>Sorry, no results. Try again.</p>}

            <Grid container justify="center">
                {filteredCabins.length > 0 && filteredCabins.map(cabin => <CabinCard cabin={cabin} />)}
            </Grid>
        </div>
    );
};
